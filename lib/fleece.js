...
function parse_github(url_path, $) {
  ...
  name = $(".entry-title > strong > a").text();
  if (name) {
...
function parse_hn(url_path, $) {
  ...
  title = $(".title > a");
  subtext = $(".subtext").text().replace(/\s+/g, " ");
  href = title.attr("href");
  if (title.text() && subtext && href) {
    subtext = subtext.replace(/\| past \| web /g, "");
...
function parse_twitter(url_path, $) {
  ...
  title = textify($("div.permalink-tweet-container p.tweet-text"), function (elems) {
...
function parse_youtube(url_path, $) {
  ...
  likes = $("#watch-header .like-button-renderer span.yt-uix-button-content").first().text().replace(/[\s,]/g, "") || 0;
  dislikes = $("#watch-header .like-button-renderer span.yt-uix-button-content").get(3);
...
function describe_url(msg_url, cb) {
  var req,
    req_len = 0;
  req = request.get(msg_url, function (err, response, body) {
    var title;
