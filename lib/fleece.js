...
function parse_github(url_path, $) {
  ...
  const description = $("div.repository-description").text().trim();
  const name = $(".entry-title > strong > a").text();
  if (name) {
...
function parse_twitter(url_path, $) {
  ...
  const username = $("div.permalink-tweet-container div.permalink-header a > span.username.js-action-profile-name > b").text();
